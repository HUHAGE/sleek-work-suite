...
const actualFS__namespace = /* @__PURE__ */ _interopNamespaceDefault(actualFS);
...
const fsFromOption = (fsOption) => !fsOption || fsOption === defaultFS || fsOption === actualFS__namespace ? defaultFS : {
  ...defaultFS,
  ...fsOption,
  promises: {
    ...defaultFS.promises,
...
class PathWin32 extends PathBase {
   ...
   */
  getRootString(path2) {
    return node_path.win32.parse(path2).root;
  }
  /**
   * @internal
   */
  ...
  try {
    ...
    for (const filePath of javaFiles) {
      ...
      if (content.includes("implements Job") || content.includes("implements org.quartz.Job")) {
        const className = path$1.basename(filePath, ".java");
...
function createWindow() {
  ...
  electron.ipcMain.handle("scan-jar-files", async (_, dirPath) => {
    try {
      ...
      return jarPaths.map((filePath) => ({
        ...
        path: path$1.dirname(filePath) + path$1.sep,
  ...
  electron.ipcMain.handle("copy-files", async (_, files) => {
    try {
      const filePaths = files.map((file) => path$1.join(file.path, file.name));
      if (process.platform === "win32") {
        ...
        `;
        const tempScriptPath = path$1.join(electron.app.getPath("temp"), "copy-files.ps1");
  ...
  electron.ipcMain.handle("scan-job-classes", async (_, dirPath) => {
    try {
  ...
  electron.ipcMain.handle("add-annotation", async (_, filePath) => {
    try {
