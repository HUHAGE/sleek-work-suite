    ...
    const response = await axios.post(
      API_URL,
      {
        ...
        messages: [
          {
            role: "system",
            content: "你是一个专业的API文档生成器。请根据提供的代码生成详细、准确、专业的API文档。包含所有必要的细节，如接口地址、请求/响应格式、参数说明、请求头、示例、返回值等。使用markdown格式输出。要求文档格式清晰，文档布局合理，文档内容丰富，呈现出专业性和美观性。注意：生成的文档内容必须严格与代码功能保持一致，不能脱离代码实际情况。"
          },
          {
            role: "user",
